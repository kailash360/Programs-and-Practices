...
def main():
    ...
    k=klv.get()
    if c:
        f_total=9/5*(int(c))+32
        k_total=int(c)+273.15
        ferh.set(str(f_total))
        klv.set(str(k_total))
    elif f:
        c_total=5/9*(int(f)-32.0)
        ke_total=5/9*(int(f)-32.0)+273
        cles.set(str(c_total))
        klv.set(str(ke_total))
    elif k:
        c_total=int(k)-273
        f_total=9 / 5 * (int(k) - 273) + 32
        cles.set(str(c_total))
        ferh.set(str(f_total))
def clear():
    cles.set("")
...
lab=Label(root,text="Help: 1.Input temperature in any scale \n2.Click Convert\n3.It will convert the values.",font=('arial',8,'bold')).grid(row=6,column=0,columnspan=3)
root.mainloop()
